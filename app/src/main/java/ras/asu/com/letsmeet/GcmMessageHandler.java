...
public class GcmMessageHandler extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (action != null && action.equalsIgnoreCase("invitation")) {
            sendNotification1(mes + " from " + from, extras.getString("title"), from);
            showToast();

        }
        if (action != null && action.equalsIgnoreCase("locUpdates")) {
            sendNotification1(mes, extras.getString("title"), from);
            sendSimpleNotification(mes,extras.getString("title"));
            showToast();
...
while(!MyNavigation.dura.toLowerCase().startsWith("1 min")) {
    ...
    Intent serv = new Intent(getApplicationContext(), MyNavigation.class);
    serv.putExtra("myLat", String.valueOf(latitude));
    serv.putExtra("myLong", String.valueOf(longitude
    ));
    serv.putExtra("pLat", extras.getString("lat").toString());
    serv.putExtra("fbId",from);
    serv.putExtra("pLong", extras.getString("longt").toString());
   getBaseContext().getApplicationContext().startService(serv);
    try {
        Thread.sleep(15000);
...
*/
        ...
        builder.setContentTitle("Navigation Updates")
                .setAutoCancel(true)
    ...
    private void sendNotification1(String msg, String title, String from) {
        /*
        Log.d("TAG", "Preparing to send notification...: " + msg);
        mNotificationManager = (NotificationManager) this
                .getSystemService(Context.NOTIFICATION_SERVICE);
...
*/
        ...
        NotificationCompat.Action cancel = new NotificationCompat.Action.Builder(R.mipmap.ic_launcher, "Cancel", pClearIntent).build();
        NotificationCompat.Action accept = new NotificationCompat.Action.Builder(R.mipmap.ic_launcher, "Accept", pColorsIntent).build();

        NotificationCompat.Builder builder;
        builder = new NotificationCompat.Builder(this).setAutoCancel(false).setPriority(Notification.PRIORITY_HIGH)
                .setContentTitle(title).setContentText(msg).setSmallIcon(R.drawable.pic3)
        ...
        Intent actionIntent = new Intent(this, MenuList.class);
        actionIntent.setAction("acceptReq");
        actionIntent.putExtra("fbID", from);
        PendingIntent actionPendingIntent = PendingIntent.getService(this, 0, actionIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        builder.addAction(R.drawable.cancel, "Cancel", actionPendingIntent);

        Intent colorsIntent = new Intent(this, MenuList.class);
        colorsIntent.setAction("cancelReq");
        colorsIntent.putExtra("fbID", from);
        PendingIntent pColorsIntent = PendingIntent.getActivity(this, 1, colorsIntent, 0);
        builder.addAction(R.drawable.accpt, "Accept", pColorsIntent);
    ...
    public void createButtonNotification (String msg, String title, String from) {
        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            // Prepare intent which is triggered if the  notification button is pressed
            Intent intent = new Intent(this, MenuList.class);
            PendingIntent pIntent = PendingIntent.getActivity(this, 0, intent, 0);
            intent.setAction("acceptReq");
            intent.putExtra("fbID", from);

            Intent intent1 = new Intent(this, MenuList.class);
            PendingIntent pIntent1 = PendingIntent.getActivity(this, 0, intent1, 0);

            intent1.setAction("cancelReq");
            intent1.putExtra("fbID", from);
            // Building the notifcation
            NotificationCompat.Builder nBuilder = new NotificationCompat.Builder(this)
                    .setSmallIcon(R.drawable.background) // notification icon
                    .setContentTitle(title) // notification title
                    .setContentText(msg) // content text
                    .setTicker("Notification") // status bar message
                    .addAction(R.drawable.accpt, "Accept", pIntent) // accept notification button
                    .addAction(R.drawable.cancel, "Cancel", pIntent1); // cancel notification button
